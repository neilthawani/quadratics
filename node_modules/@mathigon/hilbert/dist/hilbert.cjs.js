...
const SUBSUP = core.words('sub sup subsup');
...
class ExprFunction extends ExprElement {
    ...
    get variables() {
        return core.unique(core.flatten(this.args.map(a => a.variables)));
    }
    get functions() {
        return core.unique([this.fn, ...core.flatten(this.args.map(a => a.functions))]);
    }
    toString() {
        ...
        if (core.words('+ * × · / = < > ≤ ≥ ≈').includes(this.fn)) {
    ...
    toMathML(custom = {}) {
        ...
        if (core.isOneOf(this.fn, 'sup', 'sub')) {
...
class ExprTerm extends ExprElement {
    ...
    get variables() {
        return core.unique(core.join(...this.items.map(i => i.variables)));
    }
    get functions() {
...
function splitArray(items, check) {
    ...
    for (const i of items) {
        if (check(i)) {
            ...
            core.last(result).push(i);
        }
    }
    return result;
}
function isOperator(expr, fns) {
    return expr instanceof ExprOperator && core.words(fns).includes(expr.o);
}
...
function findBinaryFunction(tokens, fn) {
    ...
    if (isOperator(core.last(tokens), fn))
        throw ExprError.endOperator(core.last(tokens));
...
function matchBrackets(tokens) {
    ...
    for (const t of tokens) {
        const lastOpen = core.last(stack).length ? core.last(stack)[0].o : undefined;
        if (isOperator(t, ') ] }') || (isOperator(t, '|') && lastOpen === '|')) {
            ...
            const isFn = (isOperator(t, ')') && core.last(term) instanceof
                ExprIdentifier && core.last(term).i !== 'π');
            ...
            core.last(stack).push(t);
        }
    }
    if (stack.length > 1) {
        throw ExprError.unclosedBracket(core.last(stack)[0].o);
    }
...
function numEquals(expr1, expr2) {
    try {
        const vars = core.unique([...expr1.variables, ...expr2.variables]);
